# run_arc5_loop.ps1 -- Opus 5 build -> Fable 5 review agentic loop
# Arc 5: demo hardening (fixes to the confirmed findings from the flags-on evaluation).
#
# BOTH agents run on the claude.ai subscription.
#
# Usage:
#   cd "C:\dev\_Arkim\Arkim Procurement Agent Prototype"
#   .\loop\run_arc5_loop.ps1
#
...
#
# Handshake files:
#   DEMO_HARDENING_BRIEF.md     -- input spec, repo root (must exist)
#   DEMO_HARDENING_REPORT.md    -- builder: gate + findings
#   DEMO_HARDENING_REVIEW.md    -- reviewer: numbered findings
...
$Brief        = "DEMO_HARDENING_BRIEF.md"
$Report       = "DEMO_HARDENING_REPORT.md"
$Review       = "DEMO_HARDENING_REVIEW.md"
$Verdict      = "loop\VERDICT.txt"
$BranchPtFile = "loop\BRANCH_POINT.txt"
$ApprovalFile = "loop\AUTHORISED_TEST_EDITS.txt"
$ScriptName   = "run_arc5_loop.ps1"
...
if (-not $SkipBaseline) {
    ...
    if ($py -notmatch "$BackendBaseline passed") {
        Write-Host "Backend baseline is NOT $BackendBaseline passed. Merge arc 4b first." -ForegroundColor Red
...
function Invoke-Agent([string]$Model, [string]$Prompt, [string]$LogFile, [string]$Stage) {
    ...
    if ($out -match "(?i)hit your (session|usage) limit|rate limit|usage limit reached") {
        ...
        if ($Stage -like "BUILDER*") {
            Write-Host "  .\loop\run_arc5_loop.ps1 -SkipGate -SkipBaseline" -ForegroundColor Yellow
            Write-Host "  (the builder reads git log and continues from the last committed task)" -ForegroundColor Yellow
        } else {
            Write-Host "  .\loop\run_arc5_loop.ps1 -SkipGate -SkipFirstBuild -SkipBaseline" -ForegroundColor Yellow
        }
        exit 3
    }
}
...
$GatePrompt = @"
You are the BUILDER in a two-agent loop. Read DEMO_HARDENING_BRIEF.md in the repo root, then the
evaluation it is built on: 'git show eval/e2e-flags-on:E2E_EVAL_REPORT.md'.

$ExecRule

This turn: run the INVESTIGATION GATE ONLY (J1 through J10). Build nothing.
Write findings to DEMO_HARDENING_REPORT.md with file:line references for every claim.

J10 is critical. List every pre-existing test assertion that pins a behaviour this arc's rulings
supersede, one line per file, in EXACTLY this form:
PROPOSED TEST EDIT: <repo-relative path> :: <line ranges> :: <superseded behaviour> :: <ruling>
If there are none, write the single line: PROPOSED TEST EDITS: NONE
Be exhaustive: find them by reading the tests, not by guessing. A human approves this list before
any build begins; an assertion you miss cannot be changed later without stopping the arc.

If a scope question the brief does NOT answer blocks you, end the report with a line beginning
exactly 'GATE STOP:' followed by the question. Otherwise do NOT write that marker.

PRIME DIRECTIVE: do not modify any test, source or config file in this turn. Do not create or edit
loop/AUTHORISED_TEST_EDITS.txt -- only the human writes that file.

Commit the report on branch $Branch. Stop after the gate.
"@

$BuildPrompt = @"
You are the BUILDER. Read DEMO_HARDENING_BRIEF.md and your gate findings in DEMO_HARDENING_REPORT.md.
The eval evidence is on another branch: read files with 'git show eval/e2e-flags-on:<path>'.

$ExecRule

FIRST run 'git log --oneline' and check which of T1-T10 are already committed. A previous session
may have been cut off by a subscription limit. Build ONLY the tasks not yet committed, in order,
committing after each with a message naming the task and the finding ID. Never redo a committed task.

TEST-EDIT APPROVAL: read loop/AUTHORISED_TEST_EDITS.txt. You may modify a pre-existing test file ONLY
if it is listed there, and only under the fence: change only assertions encoding a behaviour a ruling
supersedes, REPLACE each with an assertion pinning the new behaviour (never just delete), leave every
other assertion byte-identical, keep invariants tested. Never create or edit the approval file. If a
task needs a test edit that is not approved, do not make it -- implement what you can, and record the
blocked edit under FINDINGS with file:line.

Build tests from the ACTUAL eval evidence: copy the cited evidence into new fixture files and name the
source file in each test. Never invent fixture data where evidence exists.

Hold the standard: the system must never claim more than it can prove.
- No path may substitute a listing price for an accepted quote (R1).
- The extractor's verdict may lower a match badge, never raise it (R2).
- Failing loudly must never change the request-link response (R7).

Run 'uv run pytest -q' after each backend task and 'npm test' (frontend/) after any frontend change.

When every task is committed, append to DEMO_HARDENING_REPORT.md: both final test counts as observed,
a map from each eval finding ID to the task and named tests that close it, a FINDINGS section
('None' if empty), and any new env config. Commit the report. Do not push.
"@

$FixPrompt = @"
You are the BUILDER. The reviewer returned CHANGES_REQUESTED. Read DEMO_HARDENING_REVIEW.md and fix
ONLY the numbered findings. Do not refactor anything else.

$ExecRule

The test-edit approval list in loop/AUTHORISED_TEST_EDITS.txt still governs. If a fix needs an
unapproved test edit, record it under FINDINGS instead of making it.
If loop/SCOPE_NOTES.txt exists, justify or revert each entry in the report.

Append a fix log to DEMO_HARDENING_REPORT.md per numbered finding. Run both suites, confirm green,
commit on branch $Branch. Do not push.
"@

$ReviewPrompt = @"
You are the REVIEWER, not the builder. You did not write this code and you are not here to be
agreeable. Read DEMO_HARDENING_BRIEF.md, then DEMO_HARDENING_REPORT.md, then the actual diff.
The evaluation it answers is at 'git show eval/e2e-flags-on:E2E_EVAL_REPORT.md'.

$ExecRule
Write loop/VERDICT.txt as the LAST thing you do, after every command has returned.

If the report shows the build is incomplete (tasks missing from git log), write CHANGES_REQUESTED
with a one-line review naming the missing tasks, and finish.
...
R2 and R3 are BLOCKER-class: trace EVERY place a match badge is set, and EVERY path by which an order
gets its price. Read the code; do not trust the report.

R5: for each test that claims to be built from eval evidence, run 'git show eval/e2e-flags-on:<path>'
on the cited file and confirm the fixture matches it. Invented fixtures are MAJOR.

The ONLY files you may create or modify are DEMO_HARDENING_REVIEW.md and loop/VERDICT.txt. Never fix
builder code. Do NOT commit loop/VERDICT.txt.

Write numbered findings with severity (BLOCKER/MAJOR/MINOR), file:line and brief clause. Commit only
the review file. Then write loop/VERDICT.txt containing exactly one word:
APPROVED  or  CHANGES_REQUESTED
"@

# ---- Gate -------------------------------------------------------------------
if (-not $SkipGate) {
    Write-Host "`n--- GATE: investigation (J1-J10) ---" -ForegroundColor Cyan
...
for ($i = 1; $i -le $MaxIterations; $i++) {
    ...
    elseif ($i -eq 1) {
        Write-Host "`n--- Iteration $i of $MaxIterations : BUILD (T1-T10) ---" -ForegroundColor Cyan
        Invoke-Agent $BuilderModel $BuildPrompt "build.log" "BUILDER build T1-T10"
...
Write-Host "  1. Read FINDINGS and the finding-to-test map in $Report."
...
Write-Host "  7. Re-run the evaluation on a fresh branch:  .\loop\run_e2e_eval.ps1 -Branch eval/e2e-post-hardening"
