...
| **Status** | ✅ RESOLVED (PH-01 round 3b). `RunDetail.intake_readiness` = `{ready, missing_attrs, missing_labels}` from `intake_readiness.assess` on the persisted specs (`api_server._intake_readiness_detail`). `specsReady` is deleted; the card's `ready`, kicker, *Still needed* list and the basket's *Find options* gate read only that field (absent → not ready). The *"Matching by category"* line is gone. Pinned by `test_run_detail_readiness.py`, `request-screen-readiness.test.tsx` and the source scan `request-screen-no-client-readiness.test.ts`. |
