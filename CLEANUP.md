...
| **Status** | ✅ RESOLVED (PH-01 round 3b). `RunDetail.intake_readiness` = `{ready, missing_attrs, missing_labels}` from `intake_readiness.assess` on the persisted specs (`api_server._intake_readiness_detail`). `specsReady` is deleted; the card's `ready`, kicker, *Still needed* list and the basket's *Find options* gate read only that field (absent → not ready). The *"Matching by category"* line is gone. Pinned by `test_run_detail_readiness.py`, `request-screen-readiness.test.tsx` and the source scan `request-screen-no-client-readiness.test.ts`. **Round 3c:** the review found a second client rule the 3b fix missed — `frontend/src/components/gofer/intake/spec-panel.tsx` (`hasSpecs` = manufacturer or part_number → a live *Confirm & Source* on the routed `/runs/[id]` page). It now reads `run.intake_readiness?.ready === true` (fail-closed) and a reasoned 422 shows the backend's message. The source scan now covers every file under `frontend/src` (no `specsReady`/`hasSpecs`/`*Ready(` functions; every `const ready` is the backend field; every component that calls `confirmIntake`/`useConfirmIntake` reads it). Remaining spec-field derivation on the intake surfaces: the T5b family-variant `blockSatisfied` (§5.6) — it only hides a stale family-variant question and enables no action (the family guard is deliberately outside `assess`); its fix is §5.6's. |

### 5.8 Hygienic Source anyway promised a marking nothing applied

| Field | Detail |
|---|---|
| **File** | `utils/intake_readiness.py` (`unverified_requirements`, `unverified_banner_lines`, `unverified_badge_reason`); `api_server._transform_option` / `_transform_sourcing_results`; `utils/badge_integrity.resolve` |
| **Kind** | Promise without behaviour (review_ph01_r3 finding 2). |
| **Why it existed** | The hygienic override deliberately did not set `spec_incomplete` (the identity banner's text would be false), but the banner and the exact-badge cap keyed only on `spec_incomplete`, so a hygienic-only Source anyway sourced with no banner and could badge exact — while the chat hint, the checkbox and `design/interactions.md` said the results would be marked. |
| **Status** | ✅ RESOLVED (PH-01 round 3c). One derived value, `intake_readiness.unverified_requirements(specs)` → `("identity",)` / `("hygienic",)` / both, from the existing acknowledgement records (`spec_incomplete` / `hygienic_override_ack`, which now also records `missing_labels`). The banner and the badge cap both read it. Identity-only keeps arc 5's banner and badge reason byte-identical. **Shape note:** a run overriding both keeps arc 5's banner as `specIncompleteBanner` and adds the hygienic line in `specIncompleteBannerLines` (the arc-5 S3 fixture is a hygienic-context gauge — a both run — and its pre-existing assertions pin `specIncompleteBanner == BANNER` and the identity badge reason). The banner now also renders on the proc options screen, which showed none before. Pinned by `test_ph01_round3c.py` and `unverified-banner.test.tsx`. |
